...
def _do_setup_commands(agent: str, root: Path) -> None:
    ...
    # Get commands directory path (preserve leading dot for hidden dirs like .github)
    rel_path = AGENT_COMMAND_DIRS[agent]
    ...
    if agent == "copilot":
        # Copilot uses copilot-instructions.md (single file with all instructions)
        # It does NOT support slash commands like Cursor
        instructions_file = commands_dir / "copilot-instructions.md"
        
        # Build consolidated instructions from all templates
        sections = []
        sections.append("# Context Mesh Hub Instructions for GitHub Copilot\n")
        sections.append("> These instructions help Copilot understand Context Mesh workflow.\n")
        sections.append("> Generated by `cm setup-commands --agent copilot`.\n\n")
        sections.append("## Context Mesh Workflow\n")
        sections.append("Follow the **Intent → Build → Learn** workflow:\n")
        sections.append("1. **Intent**: Define WHAT to build and WHY before coding\n")
        sections.append("2. **Build**: Implement following decisions and patterns\n")
        sections.append("3. **Learn**: Capture learnings back into context\n\n")
        sections.append("## Available Commands\n")
        sections.append("When the user asks for these actions, follow the corresponding instructions:\n\n")
        ...
        for md in sorted(templates_dir.glob("*.md")):
            ...
            command_name = md.stem  # e.g. "cm-add-feature"
            sections.append(f"### `{command_name}`\n")
            sections.append(f"**Description:** {description}\n\n")
            sections.append(body.strip())
            sections.append("\n\n---\n\n")
            count += 1
        
        # Add Context Mesh context loading instructions
        sections.append("## Context Loading\n")
        sections.append("Before any implementation, load these context files:\n")
        sections.append("- `@context/intent/project-intent.md` - Project vision\n")
        sections.append("- `@context/intent/F*.md` - Feature intents\n")
        sections.append("- `@context/decisions/D*.md` - Technical decisions\n")
        sections.append("- `@context/knowledge/patterns/` - Patterns to follow\n")
        sections.append("- `@context/knowledge/anti-patterns/` - What to avoid\n")
        
        instructions_file.write_text("".join(sections), encoding="utf-8")
        console.print(f"  [green]+[/green] {instructions_file.relative_to(root)}")
        
        print_success(f"Created [bold]{editor_name}[/bold] instructions in [bold]{instructions_file}[/bold]")
        console.print()
        console.print(f"[bold]How it works in {editor_name}:[/bold]")
        console.print("  • Copilot automatically reads .github/copilot-instructions.md")
        console.print("  • Instructions apply to ALL chat conversations in this workspace")
        console.print("  • Ask Copilot: \"add a feature for user login\" and it will follow the workflow")
        console.print()
        console.print("[dim]Note: Unlike Cursor, Copilot doesn't have /slash-commands.[/dim]")
        console.print("[dim]Instead, it uses these always-on instructions automatically.[/dim]")
        return
        ...
        # Cursor, Claude: copy .md as slash commands
        for md in sorted(templates_dir.glob("*.md")):
