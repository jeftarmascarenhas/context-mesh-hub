...
**If Context Mesh Hub MCP is available:** Call the MCP tool `cm_add_feature` with the user's input (feature name, what, why, acceptance criteria). If the user only said "add a feature" or gave a short description, ask for: feature name, what it does, why we need it, and acceptance criteria, then call the tool.

**If MCP is not available:** Load @context/.context-mesh-framework.md and @context/intent/project-intent.md. Then:
1. Check if the feature already exists in context/intent/ (feature-[name].md). If it exists, say so and suggest update-feature or a different name.
...
3. Create context/intent/feature-[name].md and context/decisions/[next-number]-[name].md with bidirectional links. Update project-intent.md Related section and changelog.
